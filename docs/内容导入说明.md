...
**当前策略：** 维护 JSONL 源文件，运行脚本校验并生成 ArkTS 内容模块  
**重要规则：** 修改 `content/exercises/*.jsonl` 后，必须重新生成 `LocalExerciseContent.ets`
...
flowchart LR
  ...
  C --> D["ContentRepository"]
  D --> E["PlanEngine.generatePlan(goal)"]
  D --> F["ExerciseDetailPage"]
  E --> G["HomePage / WorkoutPreviewPage"]
```
...
| 错误 | 处理方式 |
| --- | --- |
| 改了 JSONL 但页面没变化 | 重新运行 `node tools/content/build-content.mjs` |
